  ...
  try {
    ...
    if (checkResult.rows.length > 0) {
      return res.status(400).json({
        ...
        message:
          "Guest with this email or phone number already exists in this hotel",
      });
    }
    ...
    console.log(
      `This error occured inside the addGuest function in guest.controller.js`
    );
    res.status(500).json({
      status: "error",
...
async function getHotelIdByName(req, res) {
  try {
    ...
    let query = `SELECT id FROM hotels WHERE name=$1`;
    ...
    console.log(
      `This error occured inside the getHotelIdByName function in guest.controller.js`
    );
    res.status(500).json({
      status: "error",
      message: "Something went wrong while getting the hotel id",
      errmsg: error.message,
    });
  }
}

export { addGuest,getHotelIdByName };
